...
from src.font_fitter_engine.algo_sdf.sdf_target_space_searcher import (
...
def test_single_image(image_info, target_density):
    ...
    print(
        f"Left optimal area: {results['optimal_area_left']} (density: {results['left_achieved_density']:.2f}, diff: {results['left_density_diff']:.2f})"
    )
    print(
        f"Right optimal area: {results['optimal_area_right']} (density: {results['right_achieved_density']:.2f}, diff: {results['right_density_diff']:.2f})"
    )
    print()
    ...
    for image_info in test_images:
        for target_density in TARGET_DENSITIES:
            try:
                ...
                print(
                    f"Error processing {image_info['name']} with target density {target_density}: {e}"
                )
                print()
